...
- `gemini`: the current builtin behavior can be fully replaced by `custom`; builtin now treats `input token` as `TEXT`-modality input first, then falls back to `promptTokenCount`, and also emits `image.input/audio.input/video.input token` facts when available.
