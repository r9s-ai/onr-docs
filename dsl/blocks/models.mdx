...
```nginx
models {
  models_mode vertex;    # Default query path: /v1/projects/${credential.project_id}/locations/${channel.location}/models
}
```

The `vertex` preset requires credential metadata from the selected upstream key. Configure `credential_file` and `location` in `keys.yaml`; use `base_url_override` for regional Vertex hosts.
