...
<Callout type="info" title="Design principle">
  Runtime behavior must be described explicitly by DSL directives in `config/providers/*.conf`. The runtime proxy stays an execution engine, not a guessing layer.
</Callout>
